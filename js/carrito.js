...
function manejarRegistro(form) { form.addEventListener("submit", (e) => { e.preventDefault(); if (form.contrasena.value.length < 6) return mensaje("#mensaje-registro", "La contraseña debe tener al menos 6 caracteres.", "danger"); const rut = form.rut.value.trim(); if (!rut) { form.rut.focus(); return mensaje("#mensaje-registro", "Ingresa tu RUT para crear la cuenta.", "danger"); } const usuarios = leer(claves.usuarios, []), correo = form.correo.value.trim().toLowerCase(); if (usuarios.some((u) => u.correo === correo)) return mensaje("#mensaje-registro", "Ya existe una cuenta con este correo.", "danger"); usuarios.push({ nombre: form.nombre.value.trim(), rut, correo, contrasena: form.contrasena.value, rol: "comprador" }); guardar(claves.usuarios, usuarios); form.reset(); mensaje("#mensaje-registro", "Cuenta de comprador creada. Ya puedes iniciar sesión."); }); }
function manejarClave(form) { form.addEventListener("submit", (e) => { e.preventDefault(); if (form.nueva.value.length < 6) return mensaje("#mensaje-clave", "La nueva contraseña debe tener al menos 6 caracteres.", "danger"); const usuarios = leer(claves.usuarios, []), usuario = usuarios.find((u) => u.correo === form.correo.value.trim().toLowerCase() && u.contrasena === form.actual.value); if (!usuario) return mensaje("#mensaje-clave", "No pudimos validar tus datos.", "danger"); usuario.contrasena = form.nueva.value; guardar(claves.usuarios, usuarios); form.reset(); mensaje("#mensaje-clave", "Contraseña actualizada. Ya puedes iniciar sesión."); }); }
